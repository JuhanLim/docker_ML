...
@app.route("/")
def home():
    return "<h3> 텐서플로 자동차 연비 예측 플라스크 서비스 컨테이너"
...
def predict():
    ...
    preprocessed_payload = np.expand_dims(preprocessed_payload, axis=0) # 추가해주었음 
    prediction = model.predict(preprocessed_payload) # 대신에 밑에껄 이 코드로 대체 
